...
class RoutesTestCase(TestCase):
    ...
    def test_model_city_duplicate(self):
        try:
            ...
            self.assertEqual({'name': ['Місто with this Місто already exists.']}, e.message_dict)
    ...
    def test_model_train_duplicate(self):
        try:
            ...
            self.assertEqual({'name': ['Потяг with this Номер потягу already exists.']}, e.message_dict)   
        try:
            train = Train(name='t12', from_city=self.city_B, to_city=self.city_D, travel_time=8)
            train.full_clean()
        except ValidationError as e:
            self.assertEqual({'__all__': ['Виправте час в дорозі']}, e.message_dict)
    ...
    def test_messages_error_more_time(self):
        ...
        self.assertContains(response, 'Час в дорозі, більше заданого.', 1, 200)
